...
// Single-row settings blob, seeded via db/seed.ts; getRateMaster's
// insert-if-missing branch is only a safety net for a database that somehow
// doesn't have it yet.
export async function getRateMaster(): Promise<RateMaster> {
  const row = await queryOne<{ data: string }>('SELECT data FROM rate_master WHERE id = 1')
  if (row) return { ...DEFAULTS, ...(JSON.parse(row.data) as Partial<RateMaster>) }
  run('INSERT INTO rate_master (id, data) VALUES (1, ?)', [JSON.stringify(DEFAULTS)])
  return DEFAULTS
}
...
export async function saveRateMaster(patch: Partial<RateMaster>): Promise<RateMaster> {
  ...
  run('UPDATE rate_master SET data = ? WHERE id = 1', [JSON.stringify(updated)])
  return updated
}
