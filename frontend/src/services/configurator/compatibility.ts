...
/**
 * Compatibility engine. Reads compatibility_rules (admin-editable data, not
 * application logic) and decides whether `subjectId` in `subjectTable` is
 * allowed given the rest of the in-progress configuration.
 *
 * Semantics:
 ...
 *   AND'd across different constraint_tables. A subject with no `requires`
 *   rows at all has no requirement — it's allowed by default, subject only
 *   to `excludes`.
 */
